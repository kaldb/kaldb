...
final class DashboardsGatewayRouting {
  ...
  Upstream selectUpstream(String path) {
    ...
    // Keep OpenSearch operational node endpoints on the backing cluster, but let
    // synthetic node-info lookups resolve against Astra's compatibility API.
    if (path.startsWith("/_nodes/stats") || path.startsWith("/_nodes/http")) {
