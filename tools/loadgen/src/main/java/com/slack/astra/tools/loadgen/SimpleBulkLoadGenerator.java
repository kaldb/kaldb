...
final class SimpleBulkLoadGenerator {
  private static final DateTimeFormatter ISO_MILLIS =
      DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").withZone(ZoneOffset.UTC);

  private SimpleBulkLoadGenerator() {}

  static void runFromEnvironment() {
    final String bulkUrl = Env.get("KALDB_BULK_URL", "http://localhost:8086/_bulk");
    final String index = Env.get("INDEX", "test");
    final int batchSize = Env.getInt("BATCH_SIZE", 5);
    final double intervalSec = Env.getDouble("INTERVAL_SEC", 1.0);
    long id = Env.getLong("START_ID", 100);
    final double maxBackoff = Env.getDouble("MAX_BACKOFF", 5.0);
    ...
    while (true) {
      ...
      try {
        ...
        if (code >= 200 && code < 300) {
          ...
          System.err.printf("%nbulk POST failed: HTTP %d. Backing off %.2fs...%n", code, backoff);
          sleep(backoff);
          backoff = Math.min(maxBackoff, Math.max(intervalSec, backoff * 1.5));
        }
      } catch (IOException | InterruptedException e) {
        System.err.printf("%nbulk POST error: %s. Backing off %.2fs...%n", e.getMessage(), backoff);
        sleep(backoff);
        backoff = Math.min(maxBackoff, Math.max(intervalSec, backoff * 1.5));
      }
    }
