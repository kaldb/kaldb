...
comparisons = shared_keys.map do |key|
  old_row = old_map[key]
  new_row = new_map[key]
  next if old_row[:astra_failed] || old_row[:os_failed] || new_row[:astra_failed] || new_row[:os_failed]
...
def print_section(title, rows)
  ...
  rows.each do |row|
    puts "#{row[:name]} size=#{row[:count]} | kaldb #{row[:old_astra_ms]} -> #{row[:new_astra_ms]} ms | os #{row[:old_os_ms]} -> #{row[:new_os_ms]} ms | ratio #{format("%.2f", row[:old_ratio])} -> #{format("%.2f", row[:new_ratio])}"
  end
end
