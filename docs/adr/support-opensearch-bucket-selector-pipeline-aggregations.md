...
ClickBench-style analytical queries need SQL `HAVING`
...
Support OpenSearch bucket_selector by retaining the parsed child-pipeline tree from the coordinator's original aggregation request, excluding it from partial worker reduction, and applying it only during final distributed reduction.
